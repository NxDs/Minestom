...
public class PNode {
    ...
    public Collection<? extends PNode> getNearby(Instance instance, Set<PNode> closed, Point goal, @NotNull BoundingBox boundingBox, PPath.PathfinderCapabilities capabilities) {
        if (capabilities.type() == PPath.PathfinderType.FLYING)
            return getNearbyAir(instance, closed, goal, boundingBox, capabilities);
        else if (capabilities.type() == PPath.PathfinderType.AQUATIC)
            return getNearbyWater(instance, closed, goal, boundingBox, capabilities);
        else if (capabilities.type() == PPath.PathfinderType.AMPHIBIOUS)
            return getNearbyAmphibious(instance, closed, goal, boundingBox, capabilities);
